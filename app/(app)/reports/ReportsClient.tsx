...
// サーバーから渡された初期の法定休日リストを受け取る
export default function ReportsClient({ initialHolidays }: { initialHolidays: string[] }) {
  ...
  // サーバーから取得した初期休日でSetを初期化する（月切り替え時はAPIで再取得）
  ...
  // 月切り替え時に法定休日を再取得する（cache: no-storeでブラウザキャッシュを無効化）
  ...
  function updateLocal(date: string, field: keyof Report, value: string | number | null) {
    ...
    setReports((prev) => { const next = new Map(prev); next.set(date, updated); return next; });
  ...
  function buildRowPayload(date: string) {
    ...
    return { report_date: date, attendance_type: report.attendance_type ?? null, ...timeFields, note: report.note || null };
  }

  async function saveAll() {
    const rawInputDates = new Set<string>();
    for (const key of rawInputsRef.current.keys()) { rawInputDates.add(key.split("__")[0]); }
    const datesToSave = [...new Set([...dirtyDatesRef.current, ...rawInputDates])].filter((d) => !isFutureDate(d));
    if (datesToSave.length === 0) { setMessage("変更はありません。"); setMessageType("info"); return; }
    setSaving(true); setMessage("");
    try {
      const results = await Promise.all(
        datesToSave.map((date) => fetch("/api/reports", {
      ...
      if (failed.length > 0) {
        ...
        setMessage(data.error ?? "一部の保存に失敗しました。"); setMessageType("error"); return;
      }
      await fetchReports(); await fetchPaidLeave();
      dirtyDatesRef.current = new Set(); rawInputsRef.current = new Map();
      setDirtyDates(new Set()); setRawInputs(new Map());
      setMessage(`${datesToSave.length}件 を保存しました。`); setMessageType("info");
    } catch {
      setMessage("通信エラーが発生しました。"); setMessageType("error");
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex items-center gap-4">
        <button onClick={prevMonth} className="rounded border border-gray-400 px-3 py-1 text-sm text-gray-700 hover:bg-gray-100">前月</button>
        <h2 className="text-lg font-bold text-gray-800">{year}年{month}月</h2>
        <button onClick={nextMonth} className="rounded border border-gray-400 px-3 py-1 text-sm text-gray-700 hover:bg-gray-100">翌月</button>
        <div className="ml-auto flex items-center gap-2">
          {dirtyDates.size > 0 && (
            <span className="text-xs text-amber-600 font-medium">{dirtyDates.size}件 未保存</span>
          )}
      ...
      {message && (
        <p className={`mb-3 text-sm ${messageType === "error" ? "text-red-600" : "text-blue-600"}`}>{message}</p>
      )}
            ...
            {days.map((date) => {
              ...
              return (
                ...
                >
                  ...
                  <td className={`px-2 py-1 whitespace-nowrap font-medium ${isHoliday ? "text-red-600 font-bold" : "text-gray-900"}`}>
                    {dayNum}
                  </td>
                  ...
                  <td className="px-1 py-1">
                    <select value={report?.attendance_type ?? ""}
                      onChange={(e) => updateLocal(date, "attendance_type", e.target.value || null)}
                      disabled={future}
                      ...
                      <td key={col.key} className="px-1 py-1">
                        <div className="relative flex items-center">
                          <svg xmlns="http://www.w3.org/2000/svg"
                            ...
                            onClick={() => { if (!future) openPicker(date, col.key, report?.[col.key] as number | null); }}>
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
                          </svg>
                          <input type="text" inputMode="numeric" placeholder="HH:MM" value={displayValue}
                            onChange={(e) => {
                              setRawInputs((prev) => { const next = new Map(prev); next.set(getRawKey(date, col.key), e.target.value); return next; });
                            }}
                            onBlur={(e) => {
                              const raw = e.target.value.trim();
                              if (raw === "") { updateLocal(date, col.key, null); }
                              else { const minutes = hhmmToMinutes(raw); if (minutes !== null) updateLocal(date, col.key, minutes); }
                              setRawInputs((prev) => { const next = new Map(prev); next.delete(getRawKey(date, col.key)); return next; });
                            }}
                            disabled={future}
                          ...
                          />
                          {displayValue && !future && (
                            <button onClick={() => updateLocal(date, col.key, null)}
                              className="absolute right-1 p-1 text-gray-400 hover:text-red-500" title="時間をクリア">
                  ...
                  <td className="px-2 py-1 whitespace-nowrap text-gray-700">{formatMinutes(report?.travel_office_minutes)}</td>
                  <td className="px-2 py-1 whitespace-nowrap text-gray-700">{formatMinutes(report?.site_work_minutes)}</td>
                  <td className="px-2 py-1 whitespace-nowrap text-gray-700">{formatMinutes(report?.overtime_minutes)}</td>
                  <td className="px-2 py-1 whitespace-nowrap text-gray-700">{formatMinutes(report?.deep_night_minutes)}</td>
                  <td className="px-2 py-1 whitespace-nowrap text-gray-700">{formatMinutes(report?.holiday_work_minutes)}</td>
                  <td className="px-2 py-1">
                    <input type="text" value={report?.note ?? ""}
                      onChange={(e) => updateLocal(date, "note", e.target.value)}
                      disabled={future} placeholder="備考"
        ...
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <div className="flex flex-wrap gap-4 text-xs text-gray-700">
              {["出勤", "欠勤", "休日", "有給", "振休", "休日出勤"].map((type) => {
                const count = Array.from(reports.values()).filter((r) => r.attendance_type === type).length;
                return (
          ...
          {paidLeave !== null && paidLeave.total_granted > 0 && (
            <div className={`rounded-lg border px-4 py-2 text-xs ${
              paidLeave.remaining_days === 0 ? "border-red-300 bg-red-50"
              : paidLeave.remaining_days <= 3 ? "border-amber-300 bg-amber-50"
              : "border-green-300 bg-green-50"
            }`}>
              <div className="mb-1 font-bold text-gray-600">有給残日数</div>
              <div className="flex items-baseline gap-2">
                <span className={`text-2xl font-extrabold ${
                  paidLeave.remaining_days === 0 ? "text-red-600"
                  : paidLeave.remaining_days <= 3 ? "text-amber-600"
                  : "text-green-700"
                }`}>{paidLeave.remaining_days}</span>
                <span className="text-gray-500">日</span>
                <span className="text-gray-400">（付与 {paidLeave.total_granted}日 / 取得済 {paidLeave.used_days}日）</span>
              </div>
      ...
      {/* 時刻ダイヤル モーダル */}
      {picker && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30" onClick={() => setPicker(null)}>
          <div className="rounded-xl bg-white p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex items-center gap-3">
              <div className="flex flex-col items-center">
                ...
                <select value={picker.h} onChange={(e) => setPicker({ ...picker, h: Number(e.target.value) })}
                  className="h-32 w-16 rounded border px-1 text-center text-sm text-gray-900" size={5}>
                  {Array.from({ length: 34 }, (_, i) => (
                    <option key={i} value={i}>
                      {i <= 24 ? String(i).padStart(2, "0") : `${i}(翌${i - 24}時)`}
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="flex flex-col items-center">
                ...
                <select value={picker.m} onChange={(e) => setPicker({ ...picker, m: Number(e.target.value) })}
                  className="h-32 w-16 rounded border px-1 text-center text-sm text-gray-900" size={5}>
            ...
            <div className="mt-4 flex justify-end gap-2">
              <button onClick={() => setPicker(null)}
                className="rounded border px-4 py-1.5 text-sm text-gray-600 hover:bg-gray-50">キャンセル</button>
              <button onClick={() => { updateLocal(picker.date, picker.field, picker.h * 60 + picker.m); setPicker(null); }}
                className="rounded bg-blue-600 px-4 py-1.5 text-sm font-medium text-white hover:bg-blue-700">OK</button>
            </div>
          </div>
        </div>
      )}
